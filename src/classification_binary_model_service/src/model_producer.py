...
# Топик для задач с результатами классификации сцен от модели классификации
topic_classified_image = "classified-image"

# task_id length = .env/TASK_ID_BYTE_SIZE
async def dispatch_task_classified_image(task_id: bytes, d_model_id:bytes, c_model_id:bytes, result: bytes, loop):
    result = await producer.produce(topic=topic_classified_image, value=task_id + d_model_id + c_model_id + result, loop=loop)
    return result
