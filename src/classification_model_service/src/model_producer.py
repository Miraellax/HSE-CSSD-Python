...
async def dispatch_task_classified_image(task_id: bytes, result: bytes, loop):
    result = await producer.produce(topic=topic_classified_image, value=task_id + result, loop=loop)
    return result
